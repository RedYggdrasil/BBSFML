...
#pragma region Insert Remove
		void insert_or_assign(const TKey& InKey, const TValue& InValue)
		{
			...
			m_keysMarkedForDeletion.erase(InKey);
		}
		...
		{
			//m_keysMarkedForDeletion now is modified when we remove a value, so iterator are unvalidated
			std::unordered_set<TKey> localMarkedForDeletionCopy = m_keysMarkedForDeletion;
			removeRange(localMarkedForDeletionCopy.begin(), localMarkedForDeletionCopy.end());
			localMarkedForDeletionCopy.clear();
			...
			else
			{
				...
				for (auto it = begin; it != end; ++it)
				{
					...
					if (iter != m_IDXs.end())
					{
						...
						m_keys.erase(m_keys.begin() + foundIndex);
						m_IDXs.erase(*iter);
						m_keysMarkedForDeletion.erase(*iter);
					}
				}
